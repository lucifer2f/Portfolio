      ...
      <div className='flex items-center justify-between py-2 sm:py-2'>
        <a href="/" className='text-xl font-bold transition-colors text-neutral-400 hover:text-white'>
          ...
          <button
            ...
            <img src={isOpen ? 'assets/close.svg':'assets/menu.svg'} className='w-6 h-6' alt='toggle' loading='lazy' />
          </button>
          <nav className='hidden sm:flex'><Navigation /></nav>
        </div>
    </div>
    {isOpen && (<motion.div className='block overflow-hidden text-right sm:hidden' initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} style={{maxHeight: '100vh'}}>
      <nav className='pb-5'>
        <Navigation />
      </nav>
    </motion.div>)}
